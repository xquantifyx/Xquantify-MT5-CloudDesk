...
if [ -f /.dockerenv ] || grep -qa 'docker' /proc/1/cgroup 2>/dev/null; then
  echo "⚠ Run this on the HOST (not inside container)."; exit 1; fi
...
LAUNCHER_BIN="/usr/local/bin/mt5"
DESKTOP_FILE="/root/Desktop/MetaTrader5.desktop"
...
Install:
  ...
  --image <image>
  --mt5-url <url> | --choices-url <raw-url> [--choice <ID>] [--broker <name>]
  --debug-install      # show installer icon instead of silent install

Uninstall / cleanup:
  --uninstall          # stop & remove container only
  --purge-all          # uninstall + delete BASE_DIR (data, download, logs) + images
  --purge-data | --purge-downloads | --purge-images
  --yes                # assume yes
EOF
}
...
start_docker_daemon(){
  ...
  if command -v systemctl >/dev/null 2>&1 && [[ -d /run/systemd/system ]]; then systemctl start docker >/dev/null 2>&1 || true
  elif command -v service >/dev/null 2>&1; then service docker start >/dev/null 2>&1 || true; fi
...
# Randomize VNC pass if left default
...
if [[ "$UNINSTALL" == "1" || "$PURGE_ALL" == "1" ]]; then
  ...
  if command -v docker >/dev/null 2>&1; then
    if docker ps -a --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
      echo "   - Removing container: ${CONTAINER_NAME}"; docker rm -f "${CONTAINER_NAME}" || true
    else
      echo "   - Container not found: ${CONTAINER_NAME}"
    fi
    if [[ "$PURGE_IMAGES" == "1" ]]; then
      echo "   - Removing images:"
      echo "     > $PREFERRED_IMAGE"; docker rmi "$PREFERRED_IMAGE" >/dev/null 2>&1 || echo "       (not present)"
      echo "     > $FALLBACK_IMAGE";  docker rmi "$FALLBACK_IMAGE"  >/dev/null 2>&1 || echo "       (not present)"
    fi
  fi
  if [[ "$PURGE_ALL" == "1" ]]; then
    echo "   - Deleting BASE_DIR: $BASE_DIR"; ls -lah "$BASE_DIR" 2>/dev/null || true; rm -rf "$BASE_DIR" || true
    echo "   - Removing log: $DOCKERD_LOG"; rm -f "$DOCKERD_LOG" || true
  else
    [[ "$PURGE_DATA" == "1" && -d "$DATA_DIR" ]] && { echo "   - Deleting DATA: $DATA_DIR"; ls -lah "$DATA_DIR" || true; rm -rf "$DATA_DIR"; }
    [[ "$PURGE_DOWNLOADS" == "1" && -d "$DOWNLOAD_DIR" ]] && { echo "   - Deleting DOWNLOAD: $DOWNLOAD_DIR"; ls -lah "$DOWNLOAD_DIR" || true; rm -rf "$DOWNLOAD_DIR"; }
  fi
  echo "[DONE] Uninstall complete."; exit 0
fi
# ============================================

# Host apt small fix
[ -f /etc/apt/sources.list.d/google-chrome.list ] && mv /etc/apt/sources.list.d/google-chrome.list /etc/apt/sources.list.d/google-chrome.list.disabled || true
...
if [[ -z "$MT5_URL" ]]; then
  ...
  if [[ -n "$CHOICES" ]]; then
    ...
    mapfile -t ROWS < <(echo "$CHOICES"); declare -A ID2LINE; i=0
    ...
    echo "👉 Paste your MT5 installer URL:"; read -r -p "URL: " MT5_URL
  fi
fi
...
if [[ -f "$CACHED" ]]; then echo "[=] Use cached: $CACHED"; else echo "[+] Downloading..."; curl -fL --retry 5 --retry-all-errors -C - -o "$CACHED" "$MT5_URL"; echo "[=] Saved: $CACHED"; fi
...
DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
  ...
  ca-certificates fonts-wqy-zenhei fonts-noto-cjk desktop-file-utils
"
else
  docker exec -it "$CONTAINER_NAME" bash -lc "apt-get update && apt-get install -y -qq desktop-file-utils >/dev/null 2>&1 || true"
fi
...
# Desktop config writer (ensures no prompt + proper mime)
fix_desktop_cfg='
set -e
# 1) launcher binary
...
if [ -f \"C:\\\\Program Files\\\\MetaTrader 5\\\\terminal64.exe\" ]; then
...
# 3) pcmanfm / libfm configs to auto-exec (no prompt)
...
# 4) Update desktop MIME cache (so LXDE treats it as Application)
